  ...
  try {
    ...
    if (format === "pdf") {
      ...
      rows.forEach((row) => {
        ...
        doc.text(`Landing Page : ${row.landing_page}`);
        doc.text(`Potential : ${row.Potential}`);
        doc.text(`Enquiry : ${row.Enquiry}`);
        doc.text(`Call Back : ${row.Call_Back}`);
        doc.text(`Hindi Person : ${row.Hindi_Person}`);
        doc.text(`Sales Done With Us : ${row.Sales_Done_With_Us}`);
        doc.text(`Out Of Station : ${row.Out_Of_Station}`);
        doc.text(`Not Interested : ${row.Not_Interested}`);
        doc.text(`Genuine Reject : ${row.Genuine_Reject}`);
        doc.text(`Fraud Reject : ${row.Fraud_Reject}`);
        doc.text(`New : ${row.New_Count}`);
        doc.text(`Contacted : ${row.Contacted}`);
        doc.text(`Sales Done With Others : ${row.Sales_Done_With_Others}`);
